...
- **远程权限审批**：当 Claude Code 需要执行敏感操作（如文件写入、命令执行）时，可在手机上批准或拒绝，也可一键应用"总是允许"等权限规则
- **任务完成通知**：Claude Code 完成当前任务时，手机收到 Stop 通知并显示最后回复摘要
...
- **普通通知**（信息图标）：点击可查看详情
- **任务完成**（绿色完成图标）：Claude Code 已完成当前任务，等待新指令
...
3. 选择操作：
   - **同意** — 允许此次操作
   - **拒绝** — 拒绝此次操作
   - **允许访问目录 / 总是允许工具 / 切换模式** — 应用 Claude Code 提供的权限规则建议（按钮根据具体请求动态显示）
