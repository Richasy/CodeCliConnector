...
**典型场景**：Claude Code 在后台执行长任务，需要文件写入/命令执行等权限时，手机收到推送通知，一键批准或拒绝，无需回到电脑前。任务完成后手机也会收到通知，告知 Claude 已空闲。
...
- **权限审批**：收到权限请求时，点击通知卡片查看详情，可选择：
  - **同意** — 允许此次操作
  - **拒绝** — 拒绝此次操作
  - **允许访问目录 / 总是允许工具** — 应用 Claude Code 提供的权限规则建议
- **任务完成通知**：Claude Code 完成当前任务后，手机会收到 Stop 通知
- **设备页**：查看所有已注册设备的在线状态
- **设置页**：管理服务器连接、刷新令牌
...
3. ConnectorConsole 等待数秒（给用户在终端操作的机会）
4. 若用户未在终端操作，通过 WebSocket 转发到服务器
5. 服务器广播给所有在线终端（手机）
6. 手机收到通知，用户点击批准/拒绝（或选择"总是允许"等规则）
7. 响应原路返回到 Claude Code
8. Claude Code 继续/中止操作
9. 操作完成后 Stop Hook 发送完成通知到手机
```
